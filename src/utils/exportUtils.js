...
export function exportExcel(renters, rentRecords, filename = 'TenantPro_Export') {
...
export function exportExcelSingle(renter, rentRecords, filename) {
  ...
  XLSX.writeFile(wb, `${filename || `TenantPro_${renter.name.replace(/\s+/g, '_')}`}.xlsx`);
}

// ── Export to CSV ──────────────────────────────────────────────────────────
export function exportCSV(renters, rentRecords, filename = 'TenantPro_Export') {
  const allRows = renters.flatMap(r => buildRenterRows(r, rentRecords));
...
export function exportCSVSingle(renter, rentRecords, filename) {
  exportCSV([renter], rentRecords, filename || `TenantPro_${renter.name.replace(/\s+/g, '_')}`);
}

// ── Export to JSON ─────────────────────────────────────────────────────────
export function exportJSON(renters, rentRecords, filename = 'TenantPro_Export') {
  const data = {
...
export function exportJSONSingle(renter, rentRecords, filename) {
  ...
  downloadText(JSON.stringify(data, null, 2), `${filename || `TenantPro_${renter.name.replace(/\s+/g, '_')}`}.json`, 'application/json');
}
